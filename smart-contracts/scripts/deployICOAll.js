...
async function main() {
  ...
  const uint256Values = ['1698944400', '60000000000000000000000000'];

  // Encode the function signature
  const encodedFunctionSignature = ethers.utils.id(functionSignature).slice(0, 10);

  // Encode the addresses
  const encodedAddresses = addresses.map((address) => ethers.utils.hexZeroPad(address, 32).replace("0x", "")).join('');

  // Encode the uint256 values
  const uint256Valuesa = uint256Values.map((value) => {
    const hexValue = ethers.utils.parseUnits(value, 0); // Use 0 as the second argument for no decimals
    return hexValue
  })
  console.log(uint256Valuesa)
  const encodedUint256 = uint256Valuesa.map((address) => ethers.utils.hexZeroPad(address, 32).replace("0x", "")).join('');

  // Combine the encoded components to create the bytes string
  const bytesString = encodedFunctionSignature + encodedAddresses + encodedUint256 + ethers.utils.hexZeroPad("0x2faf8ab2b9ac8Bd4176A0B9D31502bA3a59B4b41", 32).replace("0x", "");
